(function() {
    ...
    angular
        .module('cpmApp')
        .controller('ProjectUserUploadController', ProjectUserUploadController);

    ProjectUserUploadController.$inject = ['$state','$rootScope', '$scope','Upload','previousState','AlertService','$http','$timeout','CpmUtil','$translate'];

    function ProjectUserUploadController ($state,$rootScope, $scope, Upload,previousState,AlertService,$http,$timeout,CpmUtil,$translate) {
        var vm = this;
        ...
        function downloadTpl(){
        	...
        	url += "?filePath="+encodeURI("importTpl/projectUserImportTpl.xlsx");
        	
        	window.open(url);
        }
        //上传文件
        $scope.handlingUrl = "api/project-users/uploadExcel";//处理文件的URL
        $scope.type = '4';//上传文件类型
