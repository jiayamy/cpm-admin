(function() {
    ...
    angular
        .module('cpmApp')
        .controller('ProjectInfoUploadController', ProjectInfoUploadController);

    ProjectInfoUploadController.$inject = ['$state','$rootScope', '$scope','Upload','previousState','AlertService','$http','$timeout','CpmUtil','$translate'];

    function ProjectInfoUploadController ($state,$rootScope, $scope, Upload,previousState,AlertService,$http,$timeout,CpmUtil,$translate) {
        var vm = this;
        ...
        function downloadTpl(){
        	...
        	url += "?filePath="+encodeURI("importTpl/projectInfoImportTpl.xlsx");
        	
        	window.open(url);
        }
        
        //上传文件
        $scope.handlingUrl = "api/project-infos/uploadExcel";//处理文件的URL
        $scope.type = '8';//上传文件类型
