(function() {
    ...
    angular
        .module('cpmApp')
        .controller('ContractUserUploadController', ContractUserUploadController);

    ContractUserUploadController.$inject = ['$state','$rootScope', '$scope','Upload','previousState','AlertService','$http','$timeout','CpmUtil','$translate'];

    function ContractUserUploadController ($state,$rootScope, $scope, Upload,previousState,AlertService,$http,$timeout,CpmUtil,$translate) {
        var vm = this;
        ...
        function downloadTpl(){
        	...
        	url += "?filePath="+encodeURI("importTpl/contractUserImportTpl.xlsx");
        	
        	window.open(url);
        }
        //上传文件
        $scope.handlingUrl = "api/contract-users/uploadExcel";//处理文件的URL
        $scope.type = '4';//上传文件类型
