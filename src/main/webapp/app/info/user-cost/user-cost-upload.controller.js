(function() {
    ...
    UserCostUploadController.$inject = ['$state','$rootScope', '$scope','UserCost','Upload','previousState','AlertService','$http','$timeout','CpmUtil','$translate'];

    function UserCostUploadController ($state,$rootScope, $scope, UserCost, Upload,previousState,AlertService,$http,$timeout,CpmUtil,$translate) {
        var vm = this;
        ...
        function downloadTpl(){
        	...
        	url += "?filePath="+encodeURI("importTpl/userCostImportTpl.xlsx");
        	
        	window.open(url);
        }
        
        //上传文件
        $scope.handlingUrl = "api/user-costs/uploadExcel";//处理文件的URL
        $scope.type = '2';//上传文件类型
