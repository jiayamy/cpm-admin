...
public interface DeptTypeRepository extends JpaRepository<DeptType,Long> {
	...
	@Query("select wdt from DeptType wdt, DeptInfo wdi, ContractBudget wcb, PurchaseItem wpi where wcb.deptId = wdi.id and wdi.type = wdt.id and wcb.id=wpi.budgetId and wcb.contractId = ?1 and wpi.type=?2 and wpi.source=?3 group by wdt.id ")
