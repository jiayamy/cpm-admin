...
public interface ContractInfoRepository extends JpaRepository<ContractInfo,Long> {
	...
	@Query(" from ContractInfo where createTime <= ?2 and (status = ?1 or updateTime >= ?2)")
	...
	List<ContractInfo> findByDeptTypeAndStatusOrTime(Long type,Integer status,ZonedDateTime endTime);
